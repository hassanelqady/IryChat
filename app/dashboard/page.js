  ...
  return (
    <main style={{ minHeight: '100vh', background: '#05080f' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <div style={{ display: 'flex', gap: '1rem' }}>
            ...
            <Link href="/dashboard/flows" style={{ color: '#eef2ff', textDecoration: 'none', padding: '0.5rem 0.8rem', borderRadius: '8px', fontSize: '0.9rem' }}>الردود التلقائية</Link>
            <Link href="/dashboard/analytics" style={{ color: '#eef2ff', textDecoration: 'none', padding: '0.5rem 0.8rem', borderRadius: '8px', fontSize: '0.9rem' }}>التحليلات</Link>
            <Link href="/dashboard/settings" style={{ color: '#eef2ff', textDecoration: 'none', padding: '0.5rem 0.8rem', borderRadius: '8px', fontSize: '0.9rem' }}>الإعدادات</Link>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: 'rgba(0,212,255,0.05)', padding: '0.3rem 0.8rem 0.3rem 1rem', borderRadius: '99px', border: '1px solid rgba(0,212,255,0.15)' }}>
            <span style={{ color: '#00d4ff', fontSize: '0.9rem' }}>مرحباً, {user?.email?.split('@')[0] || 'مستخدم'}</span>
      ...
      <div style={{ padding: '7rem 5% 5rem', position: 'relative', zIndex: 1 }}>
        ...
        <motion.div initial="hidden" animate="visible" variants={staggerContainer} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem', maxWidth: '1000px', margin: '0 auto 3rem' }}>
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
            <motion.div whileHover={{ y: -5, scale: 1.02 }} transition={{ duration: 0.2 }}>
              <Link href="/dashboard/accounts" style={{ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '1rem', textAlign: 'center', textDecoration: 'none', color: '#eef2ff', display: 'block' }}>
                <div style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>🔗</div>
                <div>ربط الحسابات</div>
              </Link>
            </motion.div>
            <motion.div whileHover={{ y: -5, scale: 1.02 }} transition={{ duration: 0.2 }}>
